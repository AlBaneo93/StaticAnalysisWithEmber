...
def create_vectorized_features(data_dir, feature_version=2):
    ...
    raw_feature_paths = [os.path.join(root, file) for root, dirs, files in os.walk(
        data_dir) for file in files if file.endswith("jsonl")]
    ...
    # raw_feature_paths = [os.path.join(data_dir, "test_features.jsonl")] # TODO
    # nrows = sum([1 for fp in raw_feature_paths for line in open(fp)])
    # vectorize_subset(X_path, y_path, raw_feature_paths, extractor, nrows)
...
def read_vectorized_features(data_dir, subset=None, feature_version=2):
    ...
    N = y_train.shape[0]
    X_train = np.memmap(X_train_path, dtype=np.float32,
                        mode="r", shape=(N, ndim))
    # if subset == "train":
    ...
    #     N = y_test.shape[0]
    #     X_test = np.memmap(X_test_path, dtype=np.float32,
    #                        mode="r", shape=(N, ndim))
    #     if subset == "test":
...
def create_metadata(data_dir):
    ...
    train_feature_paths = [os.path.join(
        data_dir, "train_features_{}.jsonl".format(i)) for i in range(6)]
    train_records = list(pool.imap(read_metadata_record,
                                   raw_feature_iterator(train_feature_paths)))
    train_records = [dict(record, **{"subset": "train"})
    ...
    test_records = [dict(record, **{"subset": "test"})
                    for record in test_records]

    all_metadata_keys = ["sha256", "appeared", "subset", "label", "avclass"]
    ordered_metadata_keys = [
        k for k in all_metadata_keys if k in train_records[0].keys()]
...
def train_model(data_dir, params={}, feature_version=2):
    ...
    X_train, y_train = read_vectorized_features(
        data_dir, "train", feature_version)
